var MMServer = {
	...
	, disconnect: function(socket) {
		 if(this.platforms[socket.id]) {delete this.platforms[socket.id];}
		 if(this.observers[socket.id]) {delete this.observers[socket.id];}
	...
	, init		: function(port) {
		 ...
		 this.io.on	('connection', function (socket) {
										  ...
										  socket.on	( 'mode'
													, function(data) {MMServer.mode(socket, data);}			
													);
										  socket.on	( 'update'
													, function(data) {MMServer.update(socket, data);}
													);
										  socket.on	( 'disconnect'
													, function() {MMServer.disconnect( socket );}
													);
										}
					);
		}
