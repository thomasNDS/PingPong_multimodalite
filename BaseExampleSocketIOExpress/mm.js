...
function init() {
	...
	socket.on('update', function (data) {
		 ...
		 if(!obj) {
			...
			obj.polyA = document.createElementNS('http://www.w3.org/2000/svg', 'polyline');
				obj.polyA.setAttribute('class', 'A'); obj.polyA.setAttribute('points',''); obj.svg.appendChild(obj.polyA);
			obj.polyB = document.createElementNS( 'http://www.w3.org/2000/svg', 'polyline');
				obj.polyB.setAttribute('class', 'B'); obj.polyB.setAttribute('points',''); obj.svg.appendChild(obj.polyB);
			obj.polyC = document.createElementNS( 'http://www.w3.org/2000/svg', 'polyline');
				obj.polyC.setAttribute('class', 'C'); obj.polyC.setAttribute('points',''); obj.svg.appendChild(obj.polyC);
	...
	bt_log.addEventListener('click', function() {ToggleLogger();}, false);
	document.getElementById('CmdObserve').addEventListener('click', function() {ToggleObserver();}, false);
	
	// Events
	if (window.DeviceMotionEvent) {
		 window.addEventListener('devicemotion', deviceMotionHandler, false);
		} else {document.body.appendChild(document.createTextNode("No DeviceMotionEvent"));}
	if (window.DeviceOrientationEvent) {
		 // window.addEventListener('deviceorientation', deviceOrientationHandler, false);
		} else {document.body.appendChild(document.createTextNode("No DeviceOrientationEvent"));}
}
...
function updateSVG() {
	...
	var L = ["", "", ""]
	if(this.values.length > Lg) {this.values.splice(0,1);}
	...
	if(bt.classList.contains('active')) {
		 ...
		 var div = document.createElement('div');
			...
			for(var i in T) {
				 ...
				 p.innerText = T[i]; p.setAttribute('class', T[i]);
				 p.onclick = function (str) {return function() {div.setAttribute('class', str); valType = str;};}(T[i]);
				 div.appendChild(p);
				}
		 p.onclick();
		 document.getElementById('commands').appendChild(div);
		} else {var div = document.getElementById('choiceDisplay');
